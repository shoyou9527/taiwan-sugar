            ...
            <div class="m-portlet__body" style="color: #575962;    height: calc(100% - 2.2rem - 2.2rem);padding: 1.5rem .3rem;">
                ...
                <div id="user-list" class="m-widget3 col-lg-12" style="display:inline-block;margin-bottom:10px; width:100%;padding: 0;">
                    ...
                    @foreach ($messages as $message)
                        ...
                        @endphp
                        <div class="m-widget3__item" style="margin:10px;background-color: rgba(244, 164, 164, 0.7); box-shadow: 0 1px 15px 1px rgba(244, 164, 164, 0.7); padding: 14px 28px;">
                            <div class="m-widget3__header clearfix">
                                <div class="m-widget3__user-img"> <a href="{{ route('chatWithUser', $msgUser->id) }}"><img class="m-widget3__img" style="max-width:none;width: 110px;" src="@if($msgUser->meta_()->isAvatarHidden) {{ 'makesomeerror' }} @else {{$msgUser->meta_()->pic}} @endif" onerror="this.src='/images/male-avatar.png'" alt=""></a> </div>
                                <div class="widget3_text">
                                    <div class="m-widget3__info"> <a href="{{ route('chatWithUser', $msgUser->id) }}"> <span class="m-widget3__username">{{$msgUser->name}}</span></a> </div>
                                    <p class="word_text" style="word-wrap: break-word; word-break: break-all;">{{ $message['content'] }}</p>
                                    ...
                                    {{-- <input type="submit" onclick="location.href='{!! route('deleteBetweenGET', ['uid' => $user->id, 'sid' => $msgUser->id]) !!}'" class="btn btn-danger m-btn m-btn--air m-btn--custom widget3_textbit" value="刪除"> --}}
                                </div>
                            </div>
                        </div>
                    @endforeach
